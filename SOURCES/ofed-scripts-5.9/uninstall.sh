...
my @kernel_modules = qw/srp iser isert ofa_kernel knem ummunotify mlnx-sdp mlnx-rds mlnx-nfsrdma mlnx-nvme mlnx-rdma-rxe xpmem/;
...
sub mark_for_uninstall
{
    ...
    if (not $selected_for_uninstall{$package}) {
        ...
        if ( `ofed_info 2>/dev/null | grep -i $pname 2>/dev/null` eq "" and $pname !~ /ofed-scripts|mlnx-ofed-|mlnx-fw-updater|neohost|infiniband-diags/) {
